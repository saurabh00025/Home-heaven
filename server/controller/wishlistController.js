...
const addItemInWishlist = async(req,res) => {
    ...
    try{
        ...
        if(wishlistItem){
            return res.status(200).json({success:true,message: "This item was already in your wishlist"});
        } else {
            ...
            return res.status(201).json({success:true,message: "Product addedd successfully in your wishlist"});
        }
...
const getItemInWishlistByUserId = async(req,res) => {
    // console.log("I am in get item from wishlist in backend");
